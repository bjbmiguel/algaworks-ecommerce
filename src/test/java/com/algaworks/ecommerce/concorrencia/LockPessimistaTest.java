...
public class LockPessimistaTest {
    ...
    public void misturarTiposDeLocks() {
        Runnable runnable1 = () -> {
            ...
            //Aqui a thread runnable1 faz o lock pessimista do produto com o ID = 1 para todas as outras transações
            Produto produto = entityManager1.find(Produto.class, 1, LockModeType.PESSIMISTIC_READ);

            log("Runnable 01 lista a des actual do produto = " +produto.getDescricao());
        ...
        Runnable runnable2 = () -> {
            ...
            //A thread runnable2 vai lançar uma exceção porque a thread runnable2 fe um lock no produto com PESSIMISTIC_READ
            Produto produto = entityManager2.find(Produto.class, 1, LockModeType.PESSIMISTIC_WRITE);

            log("Runnable 02 lista a des actual do produto = " +produto.getDescricao());
    ...
    public void usarLockPessimistaLockModeTypePessimisticRead() {
        Runnable runnable1 = () -> {
            ...
            log("Runnable 01 vai carregar o produto 1.");
            Produto produto = entityManager1.find(
                    Produto.class, 1, LockModeType.PESSIMISTIC_READ); //Bloqueamos o produto deixando somente o acesso a leitrua.
        ...
        esperar(1); // Para nos certificarmos que a thread1 (1º transação) vai ser executada antes da thread2..
        thread2.start();
